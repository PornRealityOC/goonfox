...
```
wget -O goonfox-__VERSION__-__RELEASE__.source.tar.gz https://gitlab.com/goonfox-community/browser/source/-/jobs/artifacts/main/raw/goonfox-__VERSION__-__RELEASE__.source.tar.gz?job=Build
curl -L -o goonfox-__VERSION__-__RELEASE__.source.tar.gz https://gitlab.com/goonfox-community/browser/source/-/jobs/artifacts/main/raw/goonfox-__VERSION__-__RELEASE__.source.tar.gz?job=Build
```
...
mkdir build
cd build
tar xf ../goonfox-__VERSION__-__RELEASE__.source.tar.gz
```
...
```
./goonfox-__VERSION__/lw/mozfetch.sh
```
This would create a _mozilla-unified_ folder in our 'build' folder, or basically anywhere that is your current working directory. It takes about an hour for me to complete, but it needs to be done only once. This step might fail and cause problems. Hack a bit, and if that fails you can ask on our [Gitter](https://gitter.im/goonfox-community/goonfox)/[Matrix](https://matrix.to/#/#goonfox:matrix.org) channels. There is no need to actually build _mozilla-unified_ (Mozilla Nightly) itself, nor is the folder needed to build Goonfox. So you can remove it: `rm -rf mozilla-unfied` if you don't plan on using/exploring it.
...
To setup the wasi sdk _headers_, you can use _goonfox-__VERSION__/lw/setup-wasi-linux.sh_. Please note that this script is a bit experimental and not all kinks have been worked out, but it should work.
...
```
cd goonfox-__VERSION__
cp lw/mozconfig.new.without-wasi mozconfig
cd ..
```
### building goonfox

Now we're ready to actually build Goonfox:
```
cd goonfox-__VERSION__
./mach build
```
...
```
git clone https://gitlab.com/goonfox-community/browser/source.git
cd source
make goonfox
```
...
## [dev info] How to use this repo instead of [Common](https://gitlab.com/goonfox-community/browser/common):
...
* _[patches](https://gitlab.com/goonfox-community/browser/common/-/tree/master/patches)_ -> _[patches](https://gitlab.com/goonfox-community/browser/source/-/tree/main/patches)_
* _[source\_files](https://gitlab.com/goonfox-community/browser/common/-/tree/master/source_files)/search-config.json_ -> _[assets](https://gitlab.com/goonfox-community/browser/source/-/tree/main/assets)/search-config.json_
* _source\_files/browser/[branding](https://gitlab.com/goonfox-community/browser/common/-/tree/master/source_files/browser/branding)/goonfox_ -> _themes/browser/[branding](https://gitlab.com/goonfox-community/browser/source/-/tree/main/themes/browser/branding)/goonfox_
...
```
git clone git@gitlab.com:goonfox-community/browser/source.git
cd source
make check
```
...
make all
```
If you have a working build environment, you can build goonfox with:
```
make goonfox
```
